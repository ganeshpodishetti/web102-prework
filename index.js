...
import { default as GAMES_DATA } from './games.js';
...
// calculate the total no of game card
const totalGames = GAMES_JSON.length;

// set inner HTML using template 
...
function showAllGames() {
    ...
    const allGames = GAMES_JSON.filter(game => game)
...
const unfundedGamesCount = GAMES_JSON.reduce((count, game) => {
    return game.pledged > game.goal ? count : count + 1;
}, 0);

const displayStr = document.createElement("div");

// create a string that explains the number of unfunded games using the ternary operator
displayStr.innerHTML = `A total of $ ${totalRaised} has been raised for ${totalGames} games. Currently, ${unfundedGamesCount} remains unfunded.
